...
class UserInfoRepository extends ServiceEntityRepository
{
    ...
    public function findUser($search = [],$isStudent=false)
    {
        // dd($search);
        ...
        if (isset($search['name'])) {
            ...
            if (sizeof($names) == 3) {
                ...
                $qb->orWhere("u.firstName LIKE '%" . $names[0] . "%' or u.middleName LIKE '%" . $names[0] . "%' or u.lastName LIKE '%" . $names[0] . "%' or u.idNumber LIKE '%" . $names[0] . "%'  ");
            }
        }
        ...
        if (isset($search['department'])) {
            $qb->andWhere('u.department = :dep')
                ->setParameter('dep', $search['department']);
        }
        if (isset($search['departments'])) {
            $qb
            //->join('u.department','d')
              ->andWhere('u.department  in( :deps)')
                ->setParameter('deps', $search['departments']);
        }
        if(!$isStudent){
            $qb->andWhere('us.isStudent = :isst')
            ->setParameter('isst',$isStudent);
        }
        if($isStudent){
            $qb->andWhere('us.isStudent = :isst')
            ->setParameter('isst',$isStudent);
    ...
    public function getStudent($search = [],$isStudent=true)
    {
        // dd($search);
        // $em = $this->getEntityManager();
        $qb = $this->createQueryBuilder('u')
